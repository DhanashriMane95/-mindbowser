...
const Home = () => {
    ...
    const getTreadingNews = async () => {
        await axios.get(`https://newsapi.org/v2/top-headlines?country=in&apikey=0d8ba95d3a974991876e0b2ff70b164c`)
            .then((response) => {
    ...
    const BookmarkStore = (data) => {
        ...
        setcolor(newState);
        localStorage.setItem('BookmarkData', JSON.stringify(newState));
    }
                                    ...
                                    <div className="col-md-4 col-12 d-flex justify-content-around" type="button" style={{ width: "26rem" }} key={index}>
                                        <div className='card border-0 py-2 my-3 shadow'>
                                            ...
                                            <div className='d-flex justify-content-between mx-2'>
                                                ...
                                                <button onClick={() => BookmarkStore(items)} className='border-0'><i className="fa-regular fa-bookmark" style={{ background: color.includes(items) ? '#ffff007a' : "" }}></i></button>
                                            </div>
                                        </div>
                    ...
                    <ul className='pagination px-4'>
                        {
                            pages.map((page, ind) => {
                                return (
                                    <li className={page === currentPage ? "page-item active" : "page-item"} key={ind}>
